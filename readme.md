...
func GetCitizenCoupon() *proto.CitizenCoupon {
	return &proto.CitizenCoupon{
		...
		TaxGroups: []*proto.TaxGroup{
			...
			{TaxRate: "E", TotalForTax: 105000, TotalTax: 18900},
		},
		TotalTax: 21500,
	}
}
...
func GetPosCoupon() *proto.PosCoupon {
	return &proto.PosCoupon{
		...
		Payments: []*proto.Payment{
			{Type: proto.PaymentType_Cash, Amount: 50000},
			{Type: proto.PaymentType_CreditCard, Amount: 100000},
			{Type: proto.PaymentType_Voucher, Amount: 32000},
		},
		Total: 182000,
		TaxGroups: []*proto.TaxGroup{
			{TaxRate: "C", TotalForTax: 45000, TotalTax: 0},
...
* **Payments** is an array of ```Payment``` that represent the types of the payment methods and the amount used by customer to pay for the goods. The valid types are: ```Cash```, ```CreditCard```, ```Voucher```, ```Cheque```, ```CryptoCurrency```, and ```Other```.
* **Total** that represents the total value to be paid by customer
* **TaxGroups** is an array of ```TaxGroup``` objects. Each ```TaxGroup``` object represents the details about tax category
* **TotalTax** is the amount of the tax in total that customer will have to pay
