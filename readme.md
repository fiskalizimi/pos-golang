...
* **CouponId** is the unique identifier of the fiscal coupon generated by POS system. CouponId has to be unique for the whole Business (across all branches)
...
* **TaxGroups** is an array of ```TaxGroup``` objects. Each ```TaxGroup``` object represents the details about tax category and has the following details:
    ...
    * **TotalForTax** - is the total amount of the all articles that fall under this Tax Rate
    * **TotalTax** is the totoal amount of Tax to be paid for this rate 
...
To use the API, you need to create new **ECDSA private key** using the **P-256 elliptic curve** and a secure random number generator. A sample code of how to generate a private key is on [pki.go](fiskalizimi/pki.go) and [main.go](fiskalizimi/main.go) files.
...
A sample code of how to generate a CSR is on [pki.go](fiskalizimi/pki.go) and [main.go](fiskalizimi/main.go) files.
...
* **verification_code:** is Verification Code (recevied during the verification step)
...
* **csr:** Tis the Certificate Signing Request in .pem format 
